  ...
  <div class="app-container">
    ...
    <div>
      ...
      <a-row type="flex" justify="space-around" align="middle">
        <a-card
          v-for="item of machineArr"
          :key="item.value.endpt"
        ...
        >
          ...
          <PieChart
            v-if="!loading"
            :pieData="beautifyPie(item.value.machineDiskInfo)"
          ></PieChart>
          <div v-else>
            <span
              v-for="(item, index) of item.value.machineDiskInfo"
              :key="index"
            >
          ...
          <PieChart
            v-if="!loading"
            :pieData="beautifyPie(item.value.machineMemInfo)"
          ></PieChart>
          <div v-else>
            <span
              v-for="(item, index) of item.value.machineMemInfo"
              :key="index"
            >
              <strong>{{ item.name + ": " }}</strong>
              <span>{{ item.value }}&nbsp</span>
          ...
          <a-button
            type="primary"
            @click="showInfo(item.value.machineCourseInfo)"
            >查看进程</a-button
          >
        </a-card>
      </a-row>
...
setTimeout(() => (checked.value = false), 15000);
...
function beautifyPie(arr) {
  ...
  return [
    { name: "未使用", value: availiable },
    { name: "已使用", value: all - availiable },
  ];
}
